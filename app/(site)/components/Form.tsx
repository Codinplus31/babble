    ...
    if (variant === "REGISTER") {
      ...
      axios
        ...
        .then((callback) => {
          ...
          if (callback?.ok) {
            // Get the current user and redirect with ID
            axios
    ...
    <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md text-[#d1d3d7] text-xl font-semibold overflow-hidden">
      <div className="rounded-xl bg-[#202c33] px-4 py-8 shadow sm:rounded-xl sm:px-10 2xl:w-full">
        <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
          {variant === "REGISTER" && (
            <Input id="name" label="Name" register={register} required errors={errors} disabled={isLoading} />
          )}
          <Input
            id="email"
            label="Email"
        ...
        <div className="mt-6">
          <div className="relative">
            ...
            <div className="relative flex justify-center text-base">
              <span className="bg-[#d1d3d7] px-2 text-[#0c1317] rounded-full">OR</span>
            </div>
          </div>
          <div className="mt-6">
            {/* Google */}
        ...
        <div className="flex gap-2 justify-center text-lg mt-6 px-2">
          <div>{variant === "LOGIN" ? "New to Babble?" : "Already a user?"}</div>
